...
class TrainConfig:
    ...
    warmup_epochs: int = 3
    head_lr: float = 5e-4
    ft_backbone_lr: float = 5e-6
    ft_head_lr: float = 2e-5
...
def run_epoch(
    ...
    progress_every = max(1, int(getattr(cfg, "progress_every", 120)))
...
def fit(project_root: Path, cfg: TrainConfig):
    ...
    checkpoint_path = checkpoint_dir / "wikiart_test7_best.pt"
    ...
    history_csv = results_dir / "wikiart_test7_history.csv"
    history_df.to_csv(history_csv, index=False)

    summary_row = {
        "notebook": "wikiart_style_classification_test7_max_accuracy.py",
        "exists": True,
        "model_name": cfg.model_name,
    ...
    summary_csv = results_dir / "wikiart_tests_1_to_7_summary.csv"
    prior_summary_csv = results_dir / "wikiart_tests_1_to_6_summary.csv"
    ...
    summary_rows = [r for r in summary_rows if str(r.get("experiment", "")).lower() != "test7"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="WikiArt style classification test7 (max accuracy).")
